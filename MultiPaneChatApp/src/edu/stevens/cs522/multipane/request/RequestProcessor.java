...
public class RequestProcessor {
	...
	public void perform(Register request, ResultReceiver resultReceiver) {
		...
		newValues.put("senderlatitude", Double.toString(request.latitude));
		newValues.put("senderlongitude", Double.toString(request.longitude));
	...
	public void perform(Synchronize sync) {
		...
		if (response.usersList != null) {
			...
			for (String[] name : response.usersList) {
				...
				new Peer(name[0], 1,Double.parseDouble(name[1]), Double.parseDouble(name[2])).writeToProvider(values);
				Log.d("inserted peer",
			...
			for (String[] msg : response.msgList) {
				...
				Log.d("JSON back to RequestProcessor", msg[0]+"--"+msg[1]+"--"+msg[2]+"--"+msg[3]+"--"+msg[4]+"--"+msg[5]+"--"+msg[6]);
				...
				if(c2!=null && c2.getCount()>0){
					...
					ContentValues values = new ContentValues();
					values.put("name", msg[5]);
					values.put("latitude", msg[2]);
					values.put("longitude", msg[3]);
					cr.insert(MessageProviderCloud.CONTENT_URI_PEER, values);
				}
				
				//ChatMessage(int id,String messageText,String sender,Double latitude, Double longitude, long messageid, long chatroomId, 
				//long date, long senderId)
				cm = new ChatMessage(0, msg[6], msg[5],Double.parseDouble(msg[2]), Double.parseDouble(msg[3]), Long.parseLong(msg[4]),Long.parseLong(chatroomId),
