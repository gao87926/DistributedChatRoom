...
public class RequestService extends IntentService {
	...
	protected void onHandleIntent(Intent intent) {
		...
		//	Log.d("RequestService triggered", "sync Room: " + chatroom);
			...
			c.close();
			Log.d("Request service geting current chatroom:",chatroom);
...
//			
			...
			sync.latitude = peer.latitude;
			sync.longitude = peer.longitude;
			
		
					
			String addr = "http://"+ prefs2.getString("hostStr", "localhost")+ ":"+prefs2.getString("portNo", "8080");
			sync.addr = addr;
			Log.d("RequestSync service", sync.addr+"==" + sync.id +"=="+ sync.userId);
			rp.perform(sync);
			break;
		case 2:
